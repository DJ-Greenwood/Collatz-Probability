...
if __name__ == "__main__":
    ...
    numbers = [start + i for i in range(1_000_000)]
...
def plot_transition_probabilities(transition_probs):
    """Plot transition probabilities as a bar chart."""
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.bar(transition_probs.keys(), transition_probs.values(), color=['blue', 'green', 'red'])
    ax.set_xlabel("Transition Type")
    ax.set_ylabel("Probability")
    ax.set_title("Collatz Transition Probabilities")
    plt.xticks(rotation=45)
    plt.show()

def plot_convergence_distribution(convergence_times, r_est, p_est):
    """Plot the empirical distribution of convergence times and compare with a fitted Negative Binomial."""
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.hist(convergence_times, bins=30, density=True, alpha=0.6, label="Empirical Data")
...
# Add visualization calls at the end of execution
if __name__ == "__main__":
    plot_transition_probabilities(transition_probs)
    plot_convergence_distribution(convergence_times, r_est, p_est)
